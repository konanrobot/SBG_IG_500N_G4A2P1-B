...
SbgErrorCode sbgProtocolInit(const char *deviceName, uint32 baudRate, SbgProtocolHandle *pHandle)
{
	...
	//
	// Check if we can store our new handle
	//
	if (pHandle)
	{
		//
		// First set our returned handle to an invalid handle
		//
		*pHandle = SBG_INVALID_PROTOCOL_HANDLE;
		...
		//
		// Check if we have opened our device
		//
...
SbgErrorCode sbgProtocolFlush(SbgProtocolHandle handle)
{
	...
	{
		//
		// Flush our serial port
		//
		sbgDeviceFlush(handle->serialHandle);

		//
		// Flush our protocol buffer
		//
...
SbgErrorCode sbgProtocolSend(SbgProtocolHandle handle, uint8 cmd, const void *pData, uint16 size)
{
	...
	{
		//
		// Check if our size is valid
		//
		if (size <= SBG_MAX_DATA_LENGTH)
		{
			//
			// Allocate our frame (8 fixed minimal frame + data size)
			//
			...
			{
				//
				// Flush our com to remove old data (re-sync)
				//
				sbgDeviceFlush(handle->serialHandle);
				...
				//
				// Calculate our crc (from CMD to end of DATA)
				//
				crc = sbgProtocolCalcCRC(pFrame+2, realSize+3);

				//
				// Fill the end of our frame
				//
...
SbgErrorCode sbgProtocolReceive(SbgProtocolHandle handle, uint8 *pCmd, void *pData, uint16 *pSize, uint16 maxSize)
{
	...
	{
		// 
		// Set our size to 0 in order to avoid possible bugs
		//
		if (pSize)
		{
			*pSize = 0;
		...
		//
		// Our max data size is SBG_MAX_DATA_LENGTH
		//
		...
		//
		// Check if we can receive some new data (our receive buffer isn't full)
		//
		...
		while (handle->serialBufferSize > 0)
		{
			...
			if (handle->serialBufferSize >= 2)
			{
				...
				for (i=0; i<handle->serialBufferSize-1; i++)
				{
					...
					{
						//
						// We have found our sync char so remove all dumy received bytes before the begining of the frame
						//
						if (i>0)
						{
			...
			if (syncFound)
			{
				...
				if (dataSize <= SBG_MAX_DATA_LENGTH)
				{
					...
					if (handle->serialBuffer[dataSize+7] == SBG_ETC)
					{
						...
						if (frameCrc == computedCrc)
						{
							...
							if (dataSize > 0)
							{
								...
								{
									//
									// Check if we have enough space to store our data field
									//
									if (dataSize > maxSize)
									{
										*pSize = maxSize;
					...
					else
					{
						...
						// Remove our SYNC and STX char because we should have an invalid sync
						//
				...
				{						
					...
					// Remove our SYNC and STX char and retry to read a valid frame
					//
					memmove(handle->serialBuffer, handle->serialBuffer+2, handle->serialBufferSize-2);
					handle->serialBufferSize = handle->serialBufferSize-2;
				}
...
SbgErrorCode sbgProtocolReceiveTimeOutMs(SbgProtocolHandle handle, uint8 *pCmd, void *pData, uint16 *pSize, uint16 maxSize, uint32 timeOutMs)
{
	...
	{
		//
		// Try to read our frame until our time out has elapsed
		//
		while (sbgGetTime() < endReceiveTime)
		{
			//
			// Max size is here set to 512 because continuous information can occur before our
			// desired answer
			//
			...
			if (errorCode == SBG_NO_ERROR)
			{
				...
				default:
					...
					// Check if our Data buffer has enough space to contain our received answer
					//
					if (sizeTmp <= maxSize)
					{
						//
					...
					{
						//
						// Our pData buffer is too small to contain our received buffer
						//
						return SBG_BUFFER_OVERFLOW;
					}
				}
		...
		//
		// No frame received during our time out so exit.
		//
		return SBG_TIME_OUT;
	}
	else
	...
	if (handle != SBG_INVALID_PROTOCOL_HANDLE)
	{
		...
		if (handle->pUserHandlerDefaultOutput)
		{
			...
			else if (handle->pUserHandlerContinuousError)
			{
				...
				// so call our continuous error callback
				//
...
SbgErrorCode sbgProtocolContinuousModeHandle(SbgProtocolHandle handle)
{
	...
	{
		//
		// We read all received frames (until we have a SBG_NOT_READY error on our receive function)
		//
		do
		{
			//
			...
			{
				//
				// Check if our received frame is a continuous frame
				//
				switch (cmd)
				{
				case SBG_CONTINUOUS_DEFAULT_OUTPUT:
				...
				default:
					//
					// Not a continuous frame we have lost this frame so return an error using our continuous error callback
					//
			...
			{
				//
				// We have an error during our frame reception so return our error using our continuous error callback
				//
				handle->pUserHandlerContinuousError(handle, errorCode, handle->pUserArgContinuousError);
			}
		...
		//
		// We have read all available frames in our buffer
		//
		errorCode = SBG_NO_ERROR;
	}
	else
