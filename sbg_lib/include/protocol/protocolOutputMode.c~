...
uint16 sbgHostToTarget16(uint8 targetOutputMode, uint16 val)
{
	...
	{
		// Our target is in little endian and our platform is in big endian
		return swap16(val);
	}
	else
	{
		// Our target is in big endian so is our platform
		return val;
	}
#elif defined SBG_PLATFORM_LITTLE_ENDIAN
	if (targetOutputMode&SBG_OUTPUT_MODE_LITTLE_ENDIAN)
	{
		// Our target is in little endian so is our platform
		return val;
	}
	else
	{
		// Our target is in big endian and our platform is in little endian
		return swap16(val);
	}
#else
...
uint32 sbgHostToTargetFloat(uint8 targetOutputMode, float val)
{
	...
	//
	// First convert our value into fixed point if needed
	//
	if (targetOutputMode&SBG_OUTPUT_MODE_FIXED)
	{
		//
		// Our target is using fixed point so we have to convert our float into fixed point format
		//
	...
	{
		//
		// Our target is using float numbers so we just have to store our float in a raw uint32
		//
		rawVal.valFloat = val;
	}
...
uint64 sbgHostToTargetDouble(uint8 targetOutputMode, double val)
{
	...
	{
		//
		// Our target is using fixed point so we have to convert our double into fixed point format
		//
...
float sbgTargetToHostFloat(uint8 targetOutputMode, uint32 val)
{
	...
	//
	// Then convert our value into float if needed
	//
	if (targetOutputMode&SBG_OUTPUT_MODE_FIXED)
	{
		//
		// Our target is using fixed point so we have to convert our fixed point into float format
		//
	...
	{
		//
		// Our target is using float numbers so we just have to return our float value
		//
