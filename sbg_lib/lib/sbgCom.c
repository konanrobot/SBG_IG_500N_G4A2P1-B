...
SbgErrorCode sbgComInit(const char *deviceName, uint32 baudRate, SbgProtocolHandle *pHandle)
{
	...
	if (errorCode == SBG_NO_ERROR)
	{
		...
		//
		// If we have an error, close our serial com port
		//
		if (errorCode != SBG_NO_ERROR)
		{
			sbgComClose(*pHandle);
...
void sbgComGetVersionAsString(char version[32])
{
	if (version)
	{
		strcpy(version, SBG_COM_VERSION);
	}
}
...
void sbgComErrorToString(SbgErrorCode errorCode, char errorMsg[256])
{
	...
	{
		//
		// For each error code, copy our error msg
		//
		switch (errorCode)
		{
		case SBG_NO_ERROR:
		...
		case SBG_INVALID_CRC:
			strcpy(errorMsg, "SBG_INVALID_CRC: Our received frame has an invalid CRC.");
			break;
		case SBG_INVALID_FRAME:
			strcpy(errorMsg, "SBG_INVALID_FRAME: Our received frame is invalid.");
			break;
		case SBG_TIME_OUT:
