...
SbgErrorCode sbgSetOdoConfig(SbgProtocolHandle handle, bool permanent, uint8 channel, SbgOdoAxis axis, float pulsesPerMeter, uint8 gainError)
{
	...
	*(uint32*)(buffer + 3*sizeof(uint8))	= sbgHostToTargetFloat(handle->targetOutputMode, pulsesPerMeter);
	buffer[3*sizeof(uint8)+sizeof(uint32)]	= gainError;
...
SbgErrorCode sbgGetOdoConfig(SbgProtocolHandle handle, uint8 channel, SbgOdoAxis *pAxis, float *pPulsesPerMeter, uint8 *pGainError)
{
	...
	//
	// Send the command used to get our external device params
	//
	...
	if (error == SBG_NO_ERROR)
	{
		...
		if (error == SBG_NO_ERROR)
		{
			...
			{
				//
				// Returns, if possible, our output parameters
				//
				if (pAxis)
				{
				...
				if (pGainError)
				{
					*pGainError = frame[2*sizeof(uint8) + sizeof(uint32)];
				}
			}
