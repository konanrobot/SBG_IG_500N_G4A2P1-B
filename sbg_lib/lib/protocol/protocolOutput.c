...
SbgErrorCode sbgFillOutputFromBuffer(uint8 targetOutputMode, uint32 outputMask, void *pBuffer, uint16 bufferSize, SbgOutput *pOutput)
{
	...
	//
	// Our ouputMask define wich output are enabled and also the order
	//
	if (pOutput)
	{
		//
		// Fill our mask each time we add a value
		//
		...
		{
			//
			// Check if we can store our data in our destination buffer
			//
			if (bufferIndex + sizeof(fixed32)*4 <= bufferSize)
			{
				//
				// Gets a pointer to store our data and increment our destination size
				//
				pBufferFixed32 = (fixed32*)((uint8*)pBuffer+bufferIndex);
				bufferIndex += sizeof(fixed32)*4;
				
				//
				// Copy and format our data to our destination buffer
				//
				for (i=0; i<4; i++)
				{
